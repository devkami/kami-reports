...
,IFNULL(CONVERT(ROUND(pdi.qtd,0),CHAR),'0') AS qtd
,IFNULL(CONVERT(ce.vl_custo_total, CHAR), '0') AS custo_total
,IFNULL(CONVERT(REPLACE(ROUND(ifnull(
  ce.vl_custo_kami, (
    SELECT cpi.preco_unit from cd_preco_item AS cpi 
    WHERE cpi.cod_produto = pdi.cod_produto 
    AND cpi.tb_preco = 'TabTbCusto'
  )
), 2),'.',','),CHAR),'0') AS custo_kami
,IFNULL(CONVERT(pdi.tb_preco, CHAR), '0') AS tb_preco
,IFNULL(CONVERT(REPLACE(ROUND(pdi.preco_venda, 2),'.',','),CHAR),'0') AS preco_unit_original
,IFNULL(CONVERT(REPLACE(ROUND((pdi.qtd * pdi.preco_venda), 2),'.',','),CHAR),'0') AS preco_total_original
,IFNULL(CONVERT(REPLACE(ROUND((((pdi.preco_venda / ce.vl_custo_total)*100)-100), 2),'.',','),CHAR),'0') AS margem_bruta
,IFNULL(CONVERT(REPLACE(ROUND(pdi.preco_total, 2),'.',','),CHAR),'0') AS preco_total
,IFNULL(CONVERT(REPLACE(ROUND((pdi.preco_total -( pdi.preco_total / pd.vl_total_produtos) * COALESCE(pd.vl_desconto,0)), 2),'.',','),CHAR),'0')  AS preco_desconto_rateado
,IFNULL(CONVERT(REPLACE(ROUND(pd.vl_total_produtos 2),'.',','),CHAR),'0') AS  vl_total_pedido
,IFNULL(CONVERT(REPLACE(ROUND((pd.vl_desconto * -1), 2),'.',','),CHAR),'0') AS  desconto_pedido
,IFNULL(CONVERT(REPLACE(ROUND(CASE WHEN nf.vl_total_nota_fiscal > 0 then nf.vl_total_nota_fiscal ELSE nf2.vl_total_nota_fiscal END, 2),'.',','),CHAR),'0') AS valor_nota
,IFNULL(CONVERT(DATE_FORMAT(pd.dt_implant, "%d/%m/%Y"), CHAR), '0') AS  dt_implante_pedido
,IFNULL(CONVERT(DATE_FORMAT(pd.dt_entrega_comprometida, "%d/%m/%Y"), CHAR), '0') AS dt_entrega_comprometida
,IFNULL(CONVERT(pd.situacao, CHAR), '0') AS situacao_entrega
,IFNULL(CONVERT(vp.descricao, CHAR), '0') AS descricao
,IFNULL(CONVERT(DATE_FORMAT(CASE WHEN nf.dt_emissao > 0 then nf.dt_emissao ELSE nf2.dt_emissao END, "%d/%m/%Y"), CHAR), '0') AS dt_faturamento
