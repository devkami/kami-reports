...
,IFNULL(CONVERT((SELECT DATE_FORMAT(MAX(nf2.dt_emissao),'%d/%m/%Y') FROM vd_nota_fiscal AS nf2 WHERE cliente.cod_cliente = nf2.cod_cliente AND nf2.situacao < 81 AND nf2.cod_empresa IN (1,2,3,4,5,6,9,10,11) AND nf2.nop IN ("6.102","6.404","BLACKFRIDAY","VENDA","VENDA_S_ESTOQUE","WORKSHOP")), CHAR), '01/01/2200') AS dt_ultima_compra
,IFNULL(CONVERT(TIMESTAMPDIFF(DAY, (SELECT MAX(nf2.dt_emissao) FROM vd_nota_fiscal AS nf2 WHERE cliente.cod_cliente = nf2.cod_cliente AND nf2.situacao < 81 AND nf2.cod_empresa IN (1,2,3,4,5,6,9,10,11) AND nf2.nop IN ("6.102","6.404","BLACKFRIDAY","VENDA","VENDA_S_ESTOQUE","WORKSHOP")), CURRENT_DATE()), CHAR), '01/01/2200') AS dias_sem_compra
