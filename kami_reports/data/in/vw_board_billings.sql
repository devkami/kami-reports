...
,IFNULL(CONVERT(nota_fiscal.vl_total_nota_fiscal, DECIMAL(10,2)), 0.0) AS valor_nota
...
,IFNULL(CONVERT(nota_fiscal.dt_emissao, DATETIME), '0000-00-00 00:00:00') AS dt_faturamento
,IFNULL(CONVERT(marca.desc_abrev, CHAR), '0') AS marca
,IFNULL(CONVERT(nota_fiscal.cod_empresa, CHAR), '0') AS cod_empresa_faturamento
,IFNULL(CONVERT(empresa_faturamento.razao_social, CHAR), '0') AS nome_empresa_faturamento
,IFNULL(CONVERT(empresa_faturamento.sigla_uf, CHAR), '0') AS uf_empresa_faturamento
...
LEFT JOIN vd_nota_fiscal AS nota_fiscal ON (nota_fiscal.cod_pedido = pd.cod_pedido AND nota_fiscal.situacao < 86 AND nota_fiscal.situacao > 79 )
...
LEFT JOIN cd_cliente_endereco AS cliente_endereco ON (cliente_endereco.cod_cliente = pd.cod_cliente)
LEFT JOIN cd_empresa AS empresa_pedido ON (empresa_pedido.cod_empresa = pd.cod_empresa)
LEFT JOIN cd_empresa AS empresa_faturamento ON (empresa_faturamento.cod_empresa = nota_fiscal.cod_empresa)
